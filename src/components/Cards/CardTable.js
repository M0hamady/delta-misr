import React from "react";
import PropTypes from "prop-types";

// components
import img1 from "assets/img/bootstrap.jpg";
import img2 from "assets/img/angular.jpg";
import img3 from "assets/img/bootstrap.jpg";
import TableDropdown from "components/Dropdowns/TableDropdown.js";
import { Link } from "react-router-dom";
function to_ex_order(order) {
  location.href =`/order/${order}`;
  console.log(order,5555555555);
}
export default function CardTable({ color, orders, table_name }) {
  function getSuccesPerc(order) {
    let allSuc = 0;
    order.component.map((comp) => {
      comp.isready ? (allSuc += 1) : allSuc;
    });
    let percentageSuc = (allSuc / order.component.length) * 100;
    return Math.round(percentageSuc);
  }

  return (
    <>
      <div
        className={
          "relative flex flex-col min-w-0 break-words w-full mb-6 shadow-lg rounded duration-150 " +
          (color === "light" ? "bg-white" : "bg-lightBlue-900 text-white")
        }
        style={{ transition: "1s" }}
      >
        <div className="rounded-t mb-0 px-4 py-3 border-0">
          <div className="flex flex-wrap items-center">
            <div className="relative w-full px-4 max-w-full flex-grow flex-1">
              <h3
                className={
                  "font-semibold text-lg " +
                  (color === "light" ? "text-blueGray-700" : "text-white")
                }
              >
                {table_name ? table_name : "table_name"}
              </h3>
            </div>
          </div>
        </div>
        <div className="block w-full overflow-x-auto">
          {/* Projects table */}
          <table className="items-center w-full bg-transparent border-collapse text-center">
            <thead>
              <tr>
                <th
                  className={
                    "px-6 align-middle border border-solid py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left " +
                    (color === "light"
                      ? "bg-blueGray-50 text-blueGray-500 border-blueGray-100"
                      : "bg-lightBlue-800 text-lightBlue-300 border-lightBlue-700")
                  }
                >
                  الاوردر
                </th>
                <th
                  className={
                    "px-6 align-middle border border-solid py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left " +
                    (color === "light"
                      ? "bg-blueGray-50 text-blueGray-500 border-blueGray-100"
                      : "bg-lightBlue-800 text-lightBlue-300 border-lightBlue-700")
                  }
                >
                  تارخ الطلب
                </th>
                <th
                  className={
                    "px-6 align-middle border border-solid py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left " +
                    (color === "light"
                      ? "bg-blueGray-50 text-blueGray-500 border-blueGray-100"
                      : "bg-lightBlue-800 text-lightBlue-300 border-lightBlue-700")
                  }
                >
                  الحالة
                </th>
                <th
                  className={
                    "px-6 align-middle border border-solid py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left " +
                    (color === "light"
                      ? "bg-blueGray-50 text-blueGray-500 border-blueGray-100"
                      : "bg-lightBlue-800 text-lightBlue-300 border-lightBlue-700")
                  }
                >
                  المكونات
                </th>
                <th
                  className={
                    "px-6 align-middle border border-solid py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left " +
                    (color === "light"
                      ? "bg-blueGray-50 text-blueGray-500 border-blueGray-100"
                      : "bg-lightBlue-800 text-lightBlue-300 border-lightBlue-700")
                  }
                >
                  تم الانتهاء من الاوردر الاحالي
                </th>
                <th
                  className={
                    "px-6 align-middle border border-solid py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left " +
                    (color === "light"
                      ? "bg-blueGray-50 text-blueGray-500 border-blueGray-100"
                      : "bg-lightBlue-800 text-lightBlue-300 border-lightBlue-700")
                  }
                ></th>
              </tr>
            </thead>
            <tbody>
              {orders.length
                ? orders.map((order) => {
                    return (
                      <tr key={order.id} onClick={()=>to_ex_order(order.id)}>
                        <th className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-left flex items-center">
                          {order.pic.length ? (
                            <img
                              src={require("assets/img/angular.jpg").default}
                              className="h-12 w-12 bg-white rounded-full border"
                              alt="..."
                            ></img>
                          ) : (
                            "no pic"
                          )}
                          <span
                            className={
                              "ml-3 font-bold " +
                              +(color === "light"
                                ? "text-blueGray-600"
                                : "text-white")
                            }
                          >
                            <Link to={`/order/${order.id}`}>{order.order}</Link>
                          </span>
                        </th>
                        <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                          <Link to={`/order/${order.id}`}>{order.date}</Link>
                        </td>
                        <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                          <i
                            className={`fas fa-circle text-${
                              getSuccesPerc(order) == 100 ? "emerald" : "orange"
                            }-500 mr-2 `}
                          ></i>
                          {getSuccesPerc(order) == 100 ? (
                            <Link to={`/order/${order.id}`}>Done </Link>
                          ) : (
                            <Link to={`/order/${order.id}`}>pending </Link>
                          )}
                        </td>
                        <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 duration-150">
                          <div className="flex">
                            {order.component.length < 5
                              ? order.component.map((compo) => {
                                  return (
                                    <label
                                    key={Math.round(
                                      Math.random() *100*Math.random()*100
                                    )}
                                      className=" font-bold text-x mr-2 border-2 border-blueGray-50 w-15 h-15 p-2 text-white rounded-full flex "
                                      style={{
                                        background: `${
                                          compo.isready
                                            ? "rgb(16, 185, 129)"
                                            : "rgb(249 115 22)"
                                        }`,
                                        alignItems: "center",
                                      }}
                                    >
                                      {compo.name}
                                    </label>
                                  );
                                })
                              : order.component.map((compo) => {
                                  if (compo.id < 5) {
                                    return (
                                      <label
                                        key={Math.round(
                                          Math.random() *100*Math.random()*100
                                        )}
                                        className=" font-bold text-x mr-2 border-2 border-blueGray-50 w-15 h-15 p-2 text-white rounded-full flex "
                                        style={{
                                          background: `${
                                            compo.isready
                                              ? "rgb(16, 185, 129)"
                                              : "rgb(249 115 22)"
                                          }`,
                                          alignItems: "center",
                                        }}
                                      >
                                        {compo.name}
                                      </label>
                                    );
                                  } else {
                                    return (
                                      <div
                                      key={Math.round(
                                        Math.random() *100*Math.random()*100
                                      )}
                                        className="w-1 h-1 mt-4 mr-1  flex  rounded-full "
                                        style={{
                                          background: `${
                                            compo.isready
                                              ? "rgb(16, 185, 129)"
                                              : "rgb(249 115 22)"
                                          }`,
                                          alignItems: "center",
                                          justifyContent: "center",
                                          width: "5px",
                                          height: "5px",
                                        }}
                                      ></div>
                                    );
                                  }
                                })}
                          </div>
                        </td>
                        <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                          <div className="flex items-center">
                            <span className="mr-2">
                              {getSuccesPerc(order)}%
                            </span>
                            <div className="relative w-full">
                              <div className="overflow-hidden h-2 text-xs flex rounded bg-red-200">
                                <div
                                  style={{
                                    width: `${getSuccesPerc(order)}%`,
                                    transition: "1s",
                                  }}
                                  className={`shadow-none flex flex-col text-center whitespace-nowrap text-white justify-center bg-${
                                    getSuccesPerc(order) >= 80
                                      ? "emerald"
                                      : getSuccesPerc(order) >= 50
                                      ? "orange"
                                      : "red"
                                  }-500`}
                                ></div>
                              </div>
                            </div>
                          </div>
                        </td>
                        <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-right">
                          <TableDropdown />
                        </td>
                      </tr>
                    );
                  })
                : "no ordeers"}
            </tbody>
          </table>
        </div>
      </div>
    </>
  );
}

CardTable.defaultProps = {
  color: "light",
};

CardTable.propTypes = {
  color: PropTypes.oneOf(["light", "dark"]),
};
